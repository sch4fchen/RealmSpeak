...
public class SendMail {
	...
	*/
	public static void sendMessage(String smtpServerAddress, String fromAddress, String toAddress, String subject, String message) throws MessagingException {
		if (smtpServerAddress == null || toAddress == null || fromAddress == null) { return; }
	...
	/**
	* Static method for convenience
	* Sends mail with a single attached "file" created with the attachmentText as the message and attachmentFileName as the filename
	* Must provide at least smtpServerAddress, fromAddress, toAddress, attachmentText, & attachmentFileName
	*/
	public static void sendMessageWithTextAttachment(String smtpServerAddress, String fromAddress, String toAddress, String subject, String message, String attachmentText, String attachmentFileName) throws MessagingException {
		if (smtpServerAddress == null || toAddress == null || fromAddress == null || attachmentText == null || attachmentFileName == null) { return; }
		if (subject == null) { subject = " "; }
		if (message == null) { message = " "; }
		SendMail mail = new SendMail(smtpServerAddress,fromAddress,toAddress);
		mail.setSubject(subject);
	...
	* Must provide at least smtpServerAddress, fromAddress, toAddress, & attachmentFilePath
	*/
	public static void sendMessageWithFileAttachment(String smtpServerAddress, String fromAddress, String toAddress, String subject, String message, String attachmentFilePath) throws MessagingException {
		if (smtpServerAddress == null || toAddress == null || fromAddress == null || attachmentFilePath == null) { return; }
		if (subject == null) { subject = " "; }
		if (message == null) { message = " "; }
		SendMail mail = new SendMail(smtpServerAddress,fromAddress,toAddress);
		mail.setSubject(subject);
	...
	*/
	public boolean addAttachmentFromFile(String attachmentFilePath) throws MessagingException {
	...
	/**
	* Add an attachment (shows up in email as a text file) with provided text contents (attachmentText) and filename (attachmentFileName)
	*/
	public boolean addAttachedTextFileFromString(String attachmentText, String attachmentFileName) throws MessagingException {
	...
	*/
	...
	public static String normalizeEmail(String email) {
		...
		if (email!=null) {
			...
			if (atSign>0 && atSign<(email.length()-1) && email.indexOf('@',atSign+1)<0) {	// Must have one and only one @ sign
				if (dot>atSign) { // Must have a dot after the at sign
					ret = email;
				}
			}
		}
