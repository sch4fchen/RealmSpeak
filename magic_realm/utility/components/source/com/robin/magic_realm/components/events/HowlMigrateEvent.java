...
public class HowlMigrateEvent implements IEvent {
	private static final String title = "Howl Migrate";
	private static final String description = "At sunset the highest numbered Howl chit is moved to an adjacent hex and summons denizens.";
	public void applyBirdsong(GameData data) {
	}
	public void applySunset(GameData data) {
		TileComponent randomTile = RealmEvents.chooseRandomTile(data);
		if (randomTile==null) {
			RealmLogging.logMessage("Event","Howl Migrate: No valid tile found.");
			return;
		}
		...
		for (GameObject go : randomTile.getHold()) {
			...
			if (chosenChit == null) {
				RealmLogging.logMessage("Event","Howl Migrate: No valid chit found.");
				return;
			}
			...
			if (tile.hasThisAttribute(RealmComponent.TILE)) {
				...
				RealmLogging.logMessage("Event","Howl Migrate: "+chosenChit.getNameWithNumber()+" is moved to "+chosenTile.getNameWithNumber()+" and summons denizens.");
			}
		}
	}
