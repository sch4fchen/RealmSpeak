...
public class PatterMigrateEvent implements IEvent {
	private static final String title = "Patter Migrate";
	...
	public void applySunset(GameData data) {
		...
		if (randomTile==null) {
			RealmLogging.logMessage("Event","Patter Migrate: No valid tile found.");
			return;
		}
		GameObject chosenChit = null;
		for (GameObject go : randomTile.getHold()) {
			RealmComponent rc = RealmComponent.getRealmComponent(go);
			if (rc.isSound() && go.getThisAttribute("sound").matches("patter") && ((StateChitComponent)rc).isFaceUp()) {
				if (chosenChit == null || go.getThisInt("clearing") > chosenChit.getThisInt("clearing")) {
					chosenChit = go;
				}
			}
			if (chosenChit == null) {
				RealmLogging.logMessage("Event","Patter Migrate: No valid chit found.");
				return;
			}
			...
			if (tile.hasThisAttribute(RealmComponent.TILE)) {
				...
				RealmLogging.logMessage("Event","Patter Migrate: "+chosenChit.getNameWithNumber()+" is moved to "+chosenTile.getNameWithNumber()+" and summons denizens.");
			}
		}
	}
